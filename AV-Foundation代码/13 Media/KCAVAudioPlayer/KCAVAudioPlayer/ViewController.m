...
-(AVAudioPlayer *)audioPlayer{
    if (!_audioPlayer) {
        ...
        //设置播放器属性
        _audioPlayer.numberOfLoops=3;//设置为0不循环；设置为负数为无限循环；设置为正整数为循环次数
        _audioPlayer.delegate=self;
        [_audioPlayer prepareToPlay];//加载音频文件到缓冲区
        if(error){
        ...
        [audioSession setCategory:AVAudioSessionCategoryPlayback error:nil];//后台播放，也是独占的(不受静音键控制)
//        [audioSession setCategory:AVAudioSessionCategoryAmbient error:nil];//混音播放，可以与其他音频应用同时播放（受静音键控制）
//        [audioSession setCategory:AVAudioSessionCategoryPlayback withOptions:AVAudioSessionCategoryOptionAllowBluetooth error:nil];//受蓝牙控制
...
-(void)routeChange:(NSNotification *)notification{
    ...
    if (changeReason==AVAudioSessionRouteChangeReasonOldDeviceUnavailable) {
        ...
        if ([portDescription.portType isEqualToString:@"Headphones"]) {
            ...
            [self.playOrPause setImage:[UIImage imageNamed:@"playing_btn_play_h"] forState:UIControlStateHighlighted];
